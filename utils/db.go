...
func DbConnect() (*gorm.DB, error) {
	...
	if dbDriver == "sqlite3" {
		pwd, _ := os.Getwd()
		dbPath := fmt.Sprintf("%s/%s", pwd, os.Getenv("SQLITE3_DB_PATH"))
		if _, err := os.Stat(dbPath); os.IsNotExist(err) {
			Log("Creating DB file...")
			_, err = os.Create(dbPath)
			if err != nil {
				LogError("Error creating SQLITE DB file:", err.Error())
				ExitApp(1)
			}
			Log("DB created successfully...")
			Log("Setting init db to true...")
			os.Setenv("INIT_DB", "true")
		} else {
			os.Setenv("INIT_DB", "false")
		}
		dbUri = dbPath
		Log("Connecting to", dbUri)
	}
